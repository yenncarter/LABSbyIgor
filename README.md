...
`--now` — дополнительно к enable сразу запускает службу Docker.
...
**docker-compose.yaml**: новый файл.
...
Что делает: Полностью останавливает и удаляет все контейнеры, сети, объёмы данных и другие ресурсы, созданные с помощью `docker-compose.yml`.
...
Что делает: Показывает статус всех контейнеров, определённых в файле docker-compose.yml.
